...
import { rosterService, type CreateShiftPayload, type CreateAssignmentPayload, type OnboardDoctorPayload } from '@/lib/services/rosterService'
...
export const useGetShifts = (options?: { enabled?: boolean }) => {
  ...
  const locationId = selectedLocationId || localStorage.getItem('rely_active_property_id') || 'default-location-id'
...
export const usePublishAssignment = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['rosterDates', companyId, locationId] })
      toast.success((data as any)?.message || 'Roster published & committed with MySQL locks!')
    },
    onError: (error: ApiError) => {
...
export const useCopyAssignment = () => {
  ...
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: { targetEffectiveFrom: string; targetEffectiveUntil: string; newRosterName?: string } }) =>
      rosterService.copyAssignment(companyId, locationId, id, payload),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['rosterDates', companyId, locationId] })
      toast.success((data as any)?.message || 'Roster copied forward successfully!')
    },
    onError: (error: ApiError) => {
