...
export const useGetResidentsForAssignment = (options?: { enabled?: boolean }) => {
  const { selectedLocationId } = useLocationStore()
  const locationId = selectedLocationId || 'all'
  const { enabled = true } = options || {}

  return useQuery({
    queryKey: ['residentsForAssignment', locationId],
    queryFn: () => assetService.getResidentsForAssignment(locationId),
    enabled: enabled && !!locationId,
  })
}
