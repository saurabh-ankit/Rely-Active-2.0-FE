...
export function SelectPersonDrawer({ isOpen, ticket, locationId, onClose, onAssignSuccess }: Props) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md max-h-[85vh] p-0 rounded-2xl bg-white border border-gray-100 shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <DialogHeader className="p-5 pb-4 border-b border-gray-100 shrink-0">
          <DialogTitle className="text-lg font-bold text-gray-900">Select a person</DialogTitle>
        </DialogHeader>

        {/* Employee List */}
        <div className="flex-1 overflow-y-auto p-4 space-y-3 pr-5">
          {isLoading ? (
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex items-center justify-end gap-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl border border-gray-200 text-xs font-bold text-gray-600 hover:bg-gray-100 cursor-pointer transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-6 py-2.5 bg-[#005390] hover:bg-[#004273] active:scale-98 text-white font-extrabold text-xs rounded-xl transition-all cursor-pointer shadow-sm disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isSubmitting ? (
              <>
