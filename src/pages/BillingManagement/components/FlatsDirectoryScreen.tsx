...
export const FlatsDirectoryScreen: React.FC = () => {
  ...
  const units = Array.isArray(unitsSummaryData?.data) ? unitsSummaryData.data : []
  ...
  const outstandingFlatsCount = units.filter(
    (u) => Number(u.financialMetrics?.totalOutstanding || 0) > 0,
  ).length
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-white border-gray-100 shadow-sm">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {units.length - occupiedFlatsCount} vacant flats available
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-gray-100 shadow-sm">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {invoices.length} invoices generated across property
            </p>
          </CardContent>
        </Card>
                        ...
                        {hasDue ? (
                          <div>
                            <span className="font-bold text-xs text-rose-600">
                              ₹{Number(unit.financialMetrics?.totalOutstanding).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-gray-100">
            <div className="flex items-center gap-3 text-xs text-gray-500">
              <span>
                ...
                of <strong className="text-gray-900">{filteredUnits.length}</strong> flats
              </span>
