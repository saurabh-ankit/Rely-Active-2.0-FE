...
export const FlatBillingDashboard: React.FC = () => {
  ...
  const subscriptions = useMemo(() => unit360Data?.data?.subscriptions || [], [unit360Data])
  const pendingEvents = useMemo(() => unit360Data?.data?.pendingEvents || [], [unit360Data])
  ...
  const billedInvoicesBySubscription = useMemo(() => {
    ...
    invoices
      .filter(
        (invoice) => invoice.status !== 'CANCELLED' && invoice.periodStart <= endDate && invoice.periodEnd >= startDate,
      )
      .forEach((invoice) => {
                    ...
                    {lastInvoicedPeriodEnd ? (
                      <div className="flex items-center gap-1.5 text-[11px] text-rose-950 bg-white/80 px-2.5 py-1.5 rounded-lg border border-rose-200/70">
                        ...
                        <span>
                          ...
                          Unbilled departure stay begins on <strong>{formatDateDDMMYYYY(nextUnbilledStartDate)}</strong>
                          .
                        </span>
                      </div>
                    ) : (
            ...
            <Card className="bg-white border-gray-200 shadow-sm">
              <CardHeader className="pb-3 border-b border-gray-100 flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  ...
                  2. Recurring Subscriptions Included ({billableSubscriptions.length})
                </CardTitle>
                <div className="flex items-center gap-2">
                  {billingMode === 'SUPPLEMENTARY' ? (
                    <>
                    ...
                    {billedInvoicesBySubscription.size > 0 && (
                      <div className="rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-800">
                        <span className="font-semibold">
                          {formatMonthDisplay(billingMonth)} subscription invoice generated.
                        </span>{' '}
