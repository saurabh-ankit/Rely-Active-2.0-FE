...
import type { BillingEventSourceModule } from '@/lib/types/billing'
...
const PRESET_CHARGES = [
  { label: '🔧 Plumbing Repair', desc: 'Plumbing Repair & Fixture Replacement', cat: 'MANUAL' as BillingEventSourceModule, defaultPrice: 350 },
  { label: '⚡ Electrical Work', desc: 'Electrical Maintenance & Fitting', cat: 'MANUAL' as BillingEventSourceModule, defaultPrice: 400 },
  { label: '🧹 Deep Cleaning', desc: 'Apartment Deep Cleaning Service', cat: 'HOUSEKEEPING' as BillingEventSourceModule, defaultPrice: 750 },
  { label: '🍽️ Guest Meal', desc: 'Guest Dining & Additional Meals', cat: 'FNB' as BillingEventSourceModule, defaultPrice: 250 },
  { label: '🚗 Transport / Cab', desc: 'Transport & Escorted Cab Facility', cat: 'TRANSPORT' as BillingEventSourceModule, defaultPrice: 500 },
  { label: '🧺 Laundry Service', desc: 'Special Fabric Laundry & Ironing', cat: 'HOUSEKEEPING' as BillingEventSourceModule, defaultPrice: 200 },
  { label: '📦 Misc Ad-hoc', desc: 'Miscellaneous Assistance / Consumables', cat: 'MANUAL' as BillingEventSourceModule, defaultPrice: 150 },
]
...
export const AddMiscellaneousChargeModal: React.FC<AddMiscellaneousChargeModalProps> = ({
  ...
  const [sourceModule, setSourceModule] = useState<BillingEventSourceModule>('MANUAL')
  ...
  useEffect(() => {
    if (open) {
      if (billingEvent) {
        setDescription(billingEvent.description || '')
        setSourceModule((billingEvent.sourceModule as BillingEventSourceModule) || 'MANUAL')
        setSelectedResidentId(billingEvent.residentId || '')
        setServiceDate(billingEvent.serviceDate ? String(billingEvent.serviceDate).split('T')[0] : todayStr)
        setQuantity(billingEvent.quantity || 1)
        setUnitPrice(billingEvent.unitPrice ?? '')
        setChargeType(billingEvent.chargeType || 'ONE_TIME')
      } else {
        const primary = occupants.find((o) => o.isPrimary) || occupants[0]
        if (primary) {
          setSelectedResidentId(primary.id)
        } else if (folio?.primaryResidentId) {
          setSelectedResidentId(folio.primaryResidentId)
        }
  ...
  }, [open, occupants, folio, billingEvent])
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-lg font-bold text-gray-900">
                Add Miscellaneous / Ad-Hoc Charge
              </DialogTitle>
          ...
          <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={sourceModule}
                  onValueChange={(val) => setSourceModule(val as BillingEventSourceModule)}
                >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="misc-date" className="text-xs font-semibold text-gray-700 flex items-center justify-between">
