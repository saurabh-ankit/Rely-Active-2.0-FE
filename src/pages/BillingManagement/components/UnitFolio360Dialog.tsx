...
import { Home, User, CreditCard, FileText, Repeat, Zap, BookOpen, PlusCircle, AlertCircle } from 'lucide-react'
...
type FolioTab = 'invoices' | 'subscriptions' | 'usage' | 'ledger' | 'parties'
...
export const UnitFolio360Dialog: React.FC<UnitFolio360DialogProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<FolioTab>('invoices')
  const { data, isLoading } = useGetUnitBilling360(unitId, isOpen && !!unitId)
        ...
        <div className="bg-gradient-to-r from-slate-900 via-[#003865] to-[#005390] text-white p-6 rounded-t-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-6 pt-4 border-t border-white/10 text-xs">
            <div className="flex items-center gap-2.5 bg-white/5 p-2.5 rounded-lg border border-white/10">
              ...
              <div>
                <span className="text-white/60 block text-[10px] uppercase font-bold tracking-wider">
                  Occupant (Who Lives Here)
                </span>
                <span className="font-semibold text-white">
                  {occupants.length > 0
                    ? occupants.map((o) => `${o.name} (${o.relationship})`).join(', ')
            ...
            <div className="flex items-center gap-2.5 bg-white/5 p-2.5 rounded-lg border border-white/10">
              ...
              <div>
                <span className="text-white/60 block text-[10px] uppercase font-bold tracking-wider">
                  Financial Sponsor (Who Pays)
                </span>
                <span className="font-semibold text-white">
                  {primaryPayer
                    ? `${primaryPayer.partyName} • ${primaryPayer.partyEmail || primaryPayer.partyPhone || primaryPayer.role}`
                    : 'Self-Pay / Folio Payer'}
                </span>
              </div>
            </div>
          </div>
          ...
          <nav className="flex space-x-6">
            {[
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id as FolioTab)}
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                          {invoices.map((inv) => (
                            <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                              <td className="px-4 py-3 font-mono font-bold text-[#005390]">{inv.invoiceNumber}</td>
                              ...
                              <td
                                className={`px-3 py-3 text-right font-bold ${Number(inv.amountDue) > 0 ? 'text-red-600' : 'text-emerald-600'}`}
                              >
                          ...
                          {subscriptions.map((sub) => (
                            <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                              <td className="px-4 py-3 font-medium text-gray-900">
                                ...
                                {sub.description && <div className="text-[10px] text-gray-400">{sub.description}</div>}
                              </td>
                              ...
                              <td className="px-3 py-3 text-gray-600 font-mono text-[11px]">{sub.billingFrequency}</td>
                              <td className="px-3 py-3 text-right font-medium text-gray-900">{sub.quantity}</td>
                              <td className="px-3 py-3 text-right font-bold text-gray-900">
                                ₹
                                {Number(sub.unitPrice * sub.quantity).toLocaleString('en-IN', {
              ...
              {activeTab === 'ledger' && (
                <div>
                  {ledgerEntries.length === 0 ? (
                    <div className="py-10 text-center text-gray-400 text-xs">No ledger transactions posted yet.</div>
                  ) : (
                    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                      <table className="min-w-full divide-y divide-gray-200 text-xs">
                        <thead className="bg-gray-50 text-gray-600 font-semibold uppercase tracking-wider">
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                          {ledgerEntries.map((entry) => (
                            <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="px-3 py-3 font-semibold text-gray-800">{entry.entryType}</td>
                              ...
                              <td className="px-4 py-3 text-right font-bold font-mono text-gray-900">
                                ₹
                                {Number(entry.runningBalance || 0).toLocaleString('en-IN', {
                                  minimumFractionDigits: 2,
                                })}
                              </td>
                            </tr>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-gray-200 grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-gray-900">
                        {folio.billingCycle} (Day {folio.billingDay})
                      </span>
                    </div>
                    <div>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                          {(folio.parties || []).map((party) => (
                            <tr key={party.id} className="hover:bg-slate-50">
                              ...
                              <td className="px-4 py-3 text-gray-600 max-w-xs truncate">{party.partyAddress || '—'}</td>
                              <td className="px-3 py-3 font-mono text-gray-600">{party.partyGstin || '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
