...
export const InvoiceDetailDialog: React.FC<InvoiceDetailDialogProps> = ({ invoiceId, isOpen, onClose }) => {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      case 'PAID':
        return (
          <Badge className="bg-emerald-100 text-emerald-800 border-emerald-300 font-bold px-2.5 py-0.5">PAID</Badge>
        )
      case 'PARTIALLY_PAID':
        return (
          <Badge className="bg-blue-100 text-blue-800 border-blue-300 font-bold px-2.5 py-0.5">PARTIALLY PAID</Badge>
        )
      case 'FINALIZED':
        return (
          <Badge className="bg-purple-100 text-purple-800 border-purple-300 font-bold px-2.5 py-0.5">FINALIZED</Badge>
        )
      case 'DRAFT':
        return <Badge className="bg-amber-100 text-amber-800 border-amber-300 font-bold px-2.5 py-0.5">DRAFT</Badge>
      case 'OVERDUE':
        return <Badge className="bg-red-100 text-red-800 border-red-300 font-bold px-2.5 py-0.5">OVERDUE</Badge>
      default:
        return (
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-4xl lg:max-w-5xl w-full max-h-[92vh] p-0 flex flex-col overflow-hidden bg-white border-gray-200 shadow-2xl rounded-2xl gap-0">
            ...
            <>
              ...
              <div className="border-b border-gray-200 pb-6">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                  ...
                  <div className="sm:text-right space-y-1">
                    ...
                    <div className="text-sm font-mono font-bold text-[#005390]">#{invoice.invoiceNumber}</div>
                    <div className="text-xs text-gray-600">
                      Billing Period:{' '}
                      <span className="font-medium text-gray-900">{formatDateDDMMYYYY(invoice.periodStart)}</span> to{' '}
                      <span className="font-medium text-gray-900">{formatDateDDMMYYYY(invoice.periodEnd)}</span>
                    </div>
                    <div className="text-xs text-gray-500">
                      Master Folio:{' '}
                      <span className="font-mono font-semibold text-gray-700">
                        {invoice.billingAccount?.accountNumber || invoice.billingAccountId}
                      </span>
                    </div>
                  </div>
                </div>
                          ...
                          <tr key={line.id || idx} className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="px-4 py-3 text-gray-900">
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                {line.chargeType && (
                                  <span className="text-[10px] text-gray-400 font-mono">• {line.chargeType}</span>
                                )}
                              </div>
                            </td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                <div className="bg-slate-50/70 p-4 rounded-xl border border-slate-200/60 text-xs space-y-2 text-gray-500">
                  ...
                  <p className="leading-relaxed">
                    2. Cheques / NEFT / RTGS transfers can be made in favour of{' '}
                    <strong>Green Valley Residency Maintenance A/c</strong>.
                  </p>
                  <p className="leading-relaxed">
        ...
        <div className="no-print invoice-dialog-footer px-6 py-3 bg-slate-50 border-t border-gray-200 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="sm" onClick={onClose} className="text-xs cursor-pointer h-8">
              Close
            </Button>
            <Button
              type="button"
