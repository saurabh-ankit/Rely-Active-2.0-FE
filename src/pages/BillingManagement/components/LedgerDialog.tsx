...
export const LedgerDialog: React.FC<LedgerDialogProps> = ({
  ...
  const entries = Array.isArray(statement) ? statement : Array.isArray(statement?.entries) ? statement.entries : []
  const creditBalance =
    statement && !Array.isArray(statement) && typeof statement.creditBalance === 'number' ? statement.creditBalance : 0
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center text-gray-400">No ledger transactions recorded yet for this folio.</div>
        ) : (
          <div className="pt-2">
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {entries.map((entry) => (
                    <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-3 py-3 whitespace-nowrap">{getEntryTypeBadge(entry.entryType)}</td>
