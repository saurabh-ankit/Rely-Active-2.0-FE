...
export const GstTaxSettingsTab: React.FC = () => {
  ...
  useEffect(() => {
    if (taxData?.data) {
      const s = taxData.data
      setGstEnabled(s.gstEnabled ?? true)
      setDefaultGstRate(Number(s.defaultGstRate ?? 18))
      setCgstRate(Number(s.cgstRate ?? 9))
      setSgstRate(Number(s.sgstRate ?? 9))
      setCompanyGstNumber(s.companyGstNumber || '')
    }
  }, [taxData])
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Configure global GST compliance, tax split ratios, and default GSTIN applied to resident billing and invoice generation.
              </p>
            </div>
          </div>
          <Button
      ...
      <div className="rounded-3xl border border-white/40 bg-white/80 p-6 shadow-lg backdrop-blur-xl space-y-8">
        ...
        {/* GST Slab & Breakdown */}
        <div className={`space-y-6 transition-opacity duration-200 ${!gstEnabled ? 'opacity-40 pointer-events-none' : ''}`}>
          {/* Rate Selection */}
